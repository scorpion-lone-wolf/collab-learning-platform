...
This document is the **master curriculum specification** for the project. In the repository, its canonical role/path is `docs/MASTER_CURRICULUM.md`. If this document is temporarily exported or shared under another filename (for example, `PRODUCTION_COLLAB_LEARNING.md`), do not maintain two divergent curriculum copies; the repository's `docs/MASTER_CURRICULUM.md` is the canonical copy used by the continuity protocol.

When two parts of this document appear to overlap, interpret them using this precedence:

1. **Teaching behavior:** the Step-by-Step Strictness Rule, Teaching Contract, Milestone Assignments, and Milestone Completion Gate are normative. A shorter summary later in the file never removes a requirement from those rules.
2. **Milestone order:** the **Authoritative Step-by-Step Milestone Sequence** is the only progression order the mentor follows. No other section defines a competing milestone order.
3. **Current project position:** `MILESTONE_STATUS.md`, `LEARNING_STATE.md`, `DECISIONS.md`, and the current repository determine where to resume. Static "initial/starting point" sections in this master curriculum apply only when the project has not started yet.
4. **Topic preservation:** the authoritative milestone sequence must preserve every required learning topic, implementation concern, verification step, and project capability defined anywhere in this curriculum unless a topic is explicitly marked optional. Reorganizing or splitting milestones must never silently remove scope.
5. **Extensions:** sections explicitly marked as extensions are additive. Sections explicitly marked optional are not prerequisites for completing the core path.
6. **Conflict fallback:** if duplicated wording differs, follow the stricter/additive requirement that preserves learning scope and verification rather than silently dropping a topic, test, assignment, or production concern.
...
# Step-by-Step Strictness Rule (Non-Negotiable, Applies to Every Milestone)

This tightens the "Learner Background" section above and binds every milestone in this document, including ones added later.

> **Nothing is "obvious." Nothing is "common sense." Nothing is skipped because it seems basic.**
>
> This rule is not limited to named technologies (NestJS, Kafka, Docker, etc.). It applies equally to:
> - general programming concepts (e.g. what a promise/async function is, what an environment variable is, what JSON is, what an HTTP status code means);
> - architecture/design vocabulary (e.g. what "idempotent" means before using the word, what a "worker" is, what a "queue" is);
> - terminal/tooling actions (e.g. what a command actually does before running it, what a flag means, what output to expect);
> - code itself (e.g. what a new syntax construct does the first time it's written, not just that it "works").
>
> Concretely, the mentor must:
> 1. Break every explanation into small, ordered steps — never a single dense paragraph covering multiple new ideas at once.
> 2. Before using any term, concept, command, or piece of syntax the learner has not already been taught in this conversation, stop and explain it in plain language first — do not use it "in passing" and explain later.
> 3. If explaining Step N would require a concept that hasn't been taught yet, teach that concept first as Step N-0, rather than assuming it or gesturing at it.
> 4. Prefer explicit, spelled-out reasoning ("we do X because Y, which means Z") over compressed or implied reasoning the learner is expected to infer.
> 5. After introducing a new concept, briefly check that its role in the current step is clear before moving to the next step, rather than stacking multiple unexplained new ideas back to back.
> 6. This applies to every milestone in this document, including newly added extension milestones. No milestone is exempt because it looks "simple" (e.g. Milestone 0/1) or "advanced" (e.g. Kubernetes/Terraform milestones) — advanced-sounding milestones get exactly the same ground-up treatment, not less.
>
> This rule does not relax the "reference briefly once already taught" allowance in the Learner Background section — once a concept has genuinely been taught step-by-step, later milestones may remind rather than re-teach. But the first encounter with any concept, anywhere in this document, must be step-by-step and must not assume the learner already knows it.
...
- production best practices;
- learner checkpoint;
- mandatory milestone assignment completed and graded according to the Required Grading Format, with no unresolved core misunderstanding unless the learner explicitly chooses to proceed despite the recorded gap.
...
When the project has not yet started, initialize the three tracking files in the bootstrap state below. The checked-in companion files may contain additional explanatory/update rules, but their actual state must remain semantically equivalent to this section until project work begins.

The tracking files must use the **Authoritative Step-by-Step Milestone Sequence** from this curriculum. Do not initialize them from any alternate or historical sequence.
...
- Progression source: Authoritative Step-by-Step Milestone Sequence
- Repository is authoritative for what implementation actually exists.
...
Milestone 0, including its architecture artifacts, checkpoint, assignment, grading, and completion gate.
...
- Explanation does not equal demonstrated understanding.
...
Begin Milestone 0 with the engineering problem and system-level mental model: compare monolith, modular monolith, and microservices; then teach bounded contexts, service/data ownership, and sync-vs-async communication before deciding boundaries or writing implementation code.
```
...
The first ADRs should emerge during Milestone 0 only after the relevant tradeoffs have been taught, discussed, and an actual durable decision has been made.

Use ADR statuses such as Proposed, Accepted, Deprecated, Superseded, and Rejected. Preserve superseded decisions rather than deleting their history.
```

These files should evolve as the project progresses. Do not pre-populate understanding, implementation, verification, or architecture decisions that have not actually happened.
...
- instructors can create coding exercises and evaluate submissions;
- the platform can perform large-scale content similarity/plagiarism analysis;
...
This section is a compact summary of the **Teaching Contract for the LLM Mentor** above. It does not override or shorten that contract.
...
7. We inspect important failure cases.
8. We review the resulting architecture.
9. We use checkpoint questions where useful to verify reasoning.
10. We give the mandatory milestone assignment and grade it using the required grading format.
11. We apply the Production-Grade Milestone Definition of Done and Milestone Completion Gate before marking the milestone complete.
12. We stop and do not begin the next milestone automatically.

Every milestone response must end with the full Teaching Contract structure:
...
A substantial session that ends before a milestone is complete must still update the persistent checkpoint files as required by the **End of Every Substantial Learning Session** section. The current milestone does not advance merely because a chat session ended.
...
## 4.5 Polyglot only when the workload justifies it

Go is not being added merely to make the architecture look more sophisticated. The mentor must treat the language choice as an engineering decision:

1. define the workload and performance target;
2. implement a representative benchmark in Node.js/TypeScript;
3. identify the actual bottleneck;
4. compare against an equivalent Go implementation;
5. choose Go only when the operational/performance benefits justify the added language and tooling;
6. document the decision as an ADR.

Node.js can execute CPU-intensive JavaScript in worker threads, so the curriculum must not teach the false rule that "Node cannot do CPU work." The real lesson is how to recognize a workload where a separate Go service provides better throughput, memory efficiency, concurrency control, or operational isolation for the measured workload.

---

## 4.6 Assume every network call can fail
...
## 4.7 At-least-once delivery means consumers must be idempotent
...
## 4.8 Observability is part of application design
...
flowchart TB
    ...
    Edge --> Assess[Assessment Service]
    ...
    Assess --> AssessmentDB[(Assessment DB)]
    ...
    VideoWorker[Video Processing Worker]
    Files <--> Kafka
    Kafka <--> VideoWorker
    VideoWorker --> S3
    VideoWorker <--> Kafka

    Runner[Code Execution / Grading Worker<br/>(Go benchmark implementation)]
    SimilarityWorker[Content Similarity / Plagiarism Worker<br/>(Go benchmark implementation)]
    Kafka <--> Runner
    Kafka <--> SimilarityWorker
    Runner --> RunnerDB[(Execution DB)]
    SimilarityWorker --> SimilarityDB[(Similarity DB)]

    Audit[Audit / Activity Consumer]
...
Video Processing Worker and CDN are covered in §30.1–§30.4 (extensions to the File Service architecture). They are shown here so the system diagram stays accurate — video is not a bolt-on, it's part of the same event-driven pipeline as every other service.
...
## 6.7.1 Full Production Payment Ledger, Refunds, and Customer Support (Extension)

This extends §6.7 and is implemented in Milestone 16.1 of the **Authoritative Step-by-Step Milestone Sequence**. Nothing above is changed. This closes the gap between "a Payment Service exists" and "money is handled the way a real production system handles it."

**1. An immutable payment ledger, not just a status field**

A production payment system does not just update one `status` column on a `payments` row and overwrite it. It keeps an **append-only ledger**:

```text
payment_transactions
  id | payment_id | type              | amount | provider_ref | created_at
  -- type: authorization | capture | refund | chargeback | adjustment
```

- every state change is a new row, never an edit of an old one — this is what makes the payment history auditable and what reconciliation (already covered) actually reconciles against;
- the `payments` table's current status is a *derived* view computed from the ledger, not a hand-edited source of truth;
- this matters because payment disputes, audits, and refunds all need to answer "what exactly happened, in what order, and why" — a single mutable status field can't answer that.

**2. A real refund state machine**
...
(or, for policy-eligible auto-refunds: requested -> processing -> completed, no manual review)
```

- refunds are not "just call the provider's refund API" — they need: a reason code (learner-requested / policy violation / support-agent-initiated / chargeback-driven), an amount (full or partial), who/what initiated it (learner self-service vs. support agent vs. automated policy), and an audit trail of every state transition;
- partial refunds must be modeled explicitly (e.g. a learner refunded 50% after using half a course) — this is a common real-world requirement, not an edge case to skip;
- a refund reaching `completed` emits a `RefundCompleted` event, which the Enrollment Service consumes to revoke course access (tying back into §30.4's access-revocation flow) and the Notification Service consumes to inform the learner.

**3. Chargebacks and disputes (the case a lot of tutorials skip)**

- the payment provider can notify you of a chargeback/dispute via webhook *after* a payment was already completed and access already granted — this is a distinct event type from a normal refund and must be handled separately;
- a chargeback should: create a ledger entry, flag the enrollment for review (not necessarily instant revocation, depending on business policy), and create a support ticket automatically so a human can investigate — chargebacks often indicate fraud and deserve a different workflow than a routine refund request.

**4. Customer Support — a real workflow, not just "admin clicks refund"**

The earlier "Admin and Back-Office Capabilities" section already lists refunds under general admin actions; this makes it a proper workflow:

```text
Learner opens a support ticket ("I want a refund for Course X")
   |
Support ticket created, linked to: learner, course, payment_id
   |
Support agent (privileged role) reviews:
   - payment ledger history
   - refund policy window (e.g. 14-day money-back guarantee)
   - course completion percentage (some platforms restrict refunds after N% consumed)
   |
Agent approves (with reason code + note) or rejects (with reason)
   |
If approved -> refund state machine above -> RefundCompleted -> access revoked -> ticket closed -> learner notified
```

- **data model:** a `support_tickets` table (learner, subject, category, linked payment/enrollment IDs, status, assigned agent, timestamps) and a `ticket_messages` table (append-only thread, same "never overwrite history" principle as the ledger);
- **authorization:** this is a good real test of the Authorization milestone's role-based access — a support agent role can view a learner's payment history and issue refunds, but must not have the same privileges as a full admin (least privilege, covered in the existing Admin section);
- **automatic policy enforcement:** self-service refund requests within the policy window (e.g. first 14 days, low completion %) can skip manual review entirely and go straight through the refund state machine — this is both better UX and less support-team load, and is the kind of business-rule-as-code decision worth discussing explicitly;
- **scope decision, stated honestly:** this can be built as a new bounded context (`support-service`) with its own database, or as a module inside the Payment Service if you want to keep the service count lean for a learning project. Either is defensible — the tradeoff is service-count/operational overhead vs. clean bounded-context separation, exactly the kind of call this document already asks you to make explicitly (see §4 Architecture Principles) rather than defaulting to either extreme.
...
## 6.9.1 Video Processing (Extension)

This extends the File Service and is covered in Milestones 24.1–24.2 without changing anything defined above.
...
- generate a poster/thumbnail image;
...
## 6.13 Assessment Service

Responsibilities:

- coding exercises and test-suite metadata;
- submission records;
- grading policy and attempt limits;
- submission state;
- learner-facing results;
- publishing `SubmissionCreated` events;
- consuming `GradingCompleted` and similarity-analysis results.

This remains a Node.js/NestJS business service. It does not execute untrusted learner code itself.

---

## 6.14 Go Code Execution / Grading Service

This is a **core polyglot learning implementation**, not an optional technology demo. The workload involves many concurrent, resource-bounded executions, so it is a strong candidate for a separate runtime. We will build and benchmark a Go implementation, but the final production language is **not predetermined**: Milestone 38.5 compares it with a representative Node.js/TypeScript implementation, and Go remains in the final architecture only if the measured operational/performance benefits justify the added language and tooling. The Kafka contract and sandbox boundary must remain stable either way.

Responsibilities:

- consume `SubmissionCreated`;
- schedule execution jobs;
- enforce per-job CPU, memory, wall-clock, output-size, and process-count limits;
- execute code in a strongly isolated sandbox;
- collect compiler/runtime/test results;
- terminate timed-out or runaway executions;
- publish `GradingCompleted`;
- expose queue depth, execution latency, timeout, and failure metrics.

Security boundary:

- the Go service itself must never treat learner-submitted source code as trusted;
- execution happens in an isolated runtime with a deliberately narrow filesystem, network policy, Linux security controls, and resource limits;
- the design must support replacing the local development sandbox with a stronger production isolation backend without changing the Kafka contract.

Scaling boundary:

- this service scales independently from the API services;
- worker capacity is driven by queued work and saturation, not simply by HTTP request rate;
- CPU-heavy workers should be isolated from latency-sensitive API workloads where the cluster design warrants it.

---

## 6.15 Go Content Similarity / Plagiarism Analysis Service

This is the second **core Go benchmark implementation** and provides a genuine CPU-intensive workload executed inside the application process rather than delegated entirely to another native tool. It is required for the learning/benchmark track so the learner can compare real implementations, but—as with the grading service—the final production language remains evidence-based and may stay Node.js/TypeScript if the benchmark does not justify Go's additional operational cost.

Responsibilities:

- consume submission/content events;
- normalize and tokenize content;
- compute shingles/fingerprints;
- run MinHash/SimHash-style similarity calculations;
- compare candidate submissions against a bounded corpus;
- optionally compare code structurally using parsed syntax trees;
- emit `SimilarityAnalysisCompleted`;
- support corpus partitioning and parallel workers;
- expose CPU time, throughput, candidate-set size, and queue-lag metrics.

This service exists specifically to teach where Go's concurrency model and predictable compiled runtime can be useful for sustained CPU-bound batch processing. The project must benchmark it against a representative Node.js implementation before the language choice is declared justified.

---

## 6.16 Audit / Activity Pipeline
...
| Code Execution| PostgreSQL metadata                   |
| Similarity    | PostgreSQL metadata                   |
| Audit         | PostgreSQL / append-only audit storage |
| Cache         | Redis                                 |
...
# 30.1 Video Transcoding and Adaptive Streaming Architecture (Extension)

This extends Section 30 and the authoritative File Service work in Milestones 23–24; the video extension itself is implemented in Milestones 24.1–24.2. Nothing above is changed.
...
S3 (raw/original)
   |
FileUploaded event (video content-type)
   |
Kafka
   |
Video Processing Worker
   ...
   +--> package as HLS (.m3u8 + segments)
   +--> generate thumbnail
   |
S3 (processed renditions)
   |
VideoProcessingCompleted / VideoProcessingFailed event
   |
Notification Service --> in-app "your video is ready" notification
Course Service --> mark lecture video as playable
```

Key ideas:

- the browser never waits on transcoding; it uploads once and gets notified later, the same pattern YouTube and most production platforms use;
- the worker is a separate deployable/scalable unit from the File Service API, since transcoding is CPU-bound and long-running while the API is request/response;
...
This is worth explaining explicitly, because it looks at first glance like Node.js — famously single-threaded — shouldn't be trusted with something as CPU-heavy as video encoding.

- FFmpeg is a separate, independently compiled native program written in C. When the NestJS worker calls it (via Node's `child_process.spawn`), it hands the actual encoding work to that separate OS process entirely. That process gets its own CPU time from the operating system scheduler — it does not run on Node's event loop or compete with it.
- Node's job in this picture is narrow and exactly what it's good at: receive a Kafka message, start the FFmpeg process, watch it (read progress from stdout/stderr, know when it exits and with what status), then do I/O afterward — upload files to S3, write rows to Postgres, publish the completion event. All of that is I/O-bound, which is precisely where Node's non-blocking event loop shines.
- Concretely: if 10 videos are transcoding at once, that's 10 separate FFmpeg OS processes each getting real CPU cores, while one Node process comfortably manages watching all 10 and handling their I/O without blocking on any of them. The "CPU-intensive part" was never actually running inside the Node process to begin with.
- The only place Node's single thread would become a real problem is if you tried to do the pixel-level video/image processing *in JavaScript itself* (e.g. a hand-rolled encoder or per-frame pixel manipulation library running in-process) — nothing in this project does that, and nothing should, since a mature native tool (FFmpeg) already solves it correctly.
- Where you do have to be careful: don't let the Node process's *own* CPU usage creep up doing things it shouldn't — e.g. parsing huge JSON progress output synchronously, or looping over file bytes in JS instead of streaming them. Keep Node doing orchestration/I/O, keep FFmpeg doing encoding, and the split stays clean.

This is the general pattern worth remembering: a language choice matters most for the work that actually executes *inside* that language's runtime. Work you hand off to a separate native process is governed by that process's own execution model, not the orchestrating language's.

---

# 30.2 Video Player Architecture (Extension)

This extends §30.1 and Section 34 (Frontend Architecture) and is implemented in Milestone 24.2. Nothing above is changed.

The Next.js video player will additionally cover:

- HLS playback via `hls.js` (or native HLS where the browser supports it, e.g. Safari);
- adaptive bitrate switching driven by measured bandwidth/buffer health, with a manual quality override in the UI;
- graceful fallback to a lower rendition (or a "buffering, switching quality" state) instead of stalling;
- playback position tracking and resume-from-last-position;
- autoplay-next-video within a course, driven by the course's lecture ordering, with a cancellable "up next" countdown UI;
- respecting course access control — the player must not be able to fetch renditions for a lecture the learner isn't enrolled in;
- graceful UI states for "processing" (video uploaded but not yet ready) vs "ready" vs "failed."

---

# 30.3 CDN Delivery, Signed Access, and Seeking Mechanics (Extension)

This extends §30.1 and §30.2 and is implemented as part of Milestone 24.2. Nothing above is changed. This section exists because "adaptive playback" alone doesn't explain *how the video actually streams properly* — that requires three more pieces.

**1. Why S3 alone is not enough — CDN in front of S3**

```text
Player requests manifest/segment
   |
CloudFront (CDN, edge cache)
   |
   +--> cache hit --> return from edge (fast, close to learner)
   +--> cache miss --> fetch from S3 (origin) --> cache it --> return
```

- serving segments directly from S3 to every learner works for a demo but is slow (single region) and expensive (repeated origin fetches) at real scale;
- a CDN caches segments at edge locations close to the learner and only hits S3 on a cache miss;
- this is the same reason YouTube/Netflix never serve video straight from their origin storage.

**2. Why per-request API auth doesn't scale — signed URLs/cookies instead**

- checking "is this learner enrolled?" against the database on *every single segment request* (there can be hundreds per video) would overwhelm the Enrollment/Course services;
- instead, when the player first requests to play a lecture, the backend does **one** authorization check, then issues a short-lived signed URL (or signed cookie, which covers every segment for that session) that the CDN itself validates — no further backend calls per segment;
- the signature expires after a short window, so a leaked link stops working quickly, and it's scoped to a specific video/lecture so it can't be reused elsewhere.

**3. How seeking and buffering actually work with HLS**

- the manifest (`.m3u8`) lists every segment and its duration; when a learner drags the seek bar, the player looks up which segment covers that timestamp and requests it directly — it does not need to download everything before that point;
- the player keeps a rolling buffer (typically 10–30 seconds ahead) and continuously downloads the next segment slightly before it's needed;
- if a segment download is slow, the player either drops to a lower-quality rendition's version of that segment (adaptive bitrate reacting to a slow network) or shows a buffering spinner if it has no better option — this is the runtime behavior that makes "auto fallback to quality based on bandwidth" actually happen, not just a theoretical setting.

Milestone 24.2 will implement all three of these, not just the player UI — CDN configuration and signed-URL issuance are part of the backend side of that milestone.

---

# 30.4 Payment-Gated Video Access — End-to-End Security (Extension)

This extends §30.3 and directly answers: *"only a user who paid should be able to watch."* It ties the video pipeline back to the authoritative Payment (Milestone 16), Enrollment (Milestone 7), and Saga (Milestones 17–19, especially the orchestrated purchase flow in Milestone 18) work already defined elsewhere in this document. Nothing above is changed.

**1. Where the actual payment check happens**

```text
Learner clicks "Play" on a lecture
   |
Backend: "does this learner have an active EnrollmentCreated
          for this course?" (checked against Enrollment Service,
          not trusted from the client)
   |
   +--> NOT enrolled --> reject, no signed URL issued
   |
   +--> enrolled --> issue short-lived signed URL/cookie for this lecture
                       |
                       CDN validates the signature on every segment request
                       (no further backend/database call per segment)
```

- the check is against the Enrollment Service's own state, which only exists because of a successful `PaymentSucceeded` → `EnrollmentCreated` flow from the Payment/Saga milestones — a learner can't fake enrollment from the client, since the client never controls this state;
- this is the single choke point: everything downstream (CDN, segments, player) trusts the signature, not the learner's claim of having paid.

**2. What happens when access should stop (refund/cancellation)**

- when a refund succeeds, the Enrollment Service marks enrollment as revoked (this is already covered by the Saga/compensation milestones);
- because signed URLs/cookies are short-lived (minutes, not hours), revocation doesn't require actively killing an in-progress session — it simply means the *next* request for a new signed URL is rejected; any already-issued URL naturally expires within minutes;
- this is a deliberate tradeoff: simpler than building real-time session revocation, acceptable because the exposure window is small and bounded by the token TTL.

**3. Preventing bypass of the signed URL entirely**

- the S3 bucket holding processed video must **not** be publicly readable — it should only be reachable through the CDN, enforced via an Origin Access Control/Origin Access Identity, so nobody can skip the signed-URL check by guessing or finding a direct S3 object key;
- this is a common real-world misconfiguration to call out explicitly: a "secure" signed-URL system is worthless if the origin bucket is still publicly reachable underneath it.

**4. The honest security boundary — what signed URLs do *not* solve**

- a signed URL stops someone from watching *without paying* (no valid session = no playable stream);
- it does **not** stop a paying learner from screen-recording the video, downloading segments during a valid session and reassembling them, or sharing their account login;
- solving *that* class of problem is DRM (e.g. Widevine/FairPlay/PlayReady license-based encryption) and/or forensic watermarking — genuinely complex, licensing-heavy systems that are explicitly **out of scope for the core project** (see §81 Optional Advanced Extensions) but worth naming so it's clear what "secure" means here: *access-gated*, not *copy-proof*. Every mainstream ed-tech platform without enterprise DRM has this same boundary.
...
# 76. Important End-to-End Workflows We Will Build
...
## Workflow C.1 — Refund / Chargeback and Access Revocation (Extension)

```text
Learner or Support Agent
 -> refund request / dispute event
 -> Payment Service ledger + refund/dispute state machine
 -> Payment Provider
 -> RefundCompleted (or chargeback review state)
      -> Enrollment Service revokes/updates entitlement when policy requires
      -> Notification Service informs learner
      -> Audit / Activity Pipeline records privileged/security-sensitive transitions
```

The support-ticket workflow, partial refunds, policy-based auto-refunds, and chargebacks remain distinct states rather than being collapsed into one mutable payment status.
...
## Workflow D.1 — Video Upload and Adaptive Playback (Extension)

```text
Instructor uploads lecture video
 -> Browser -> pre-signed URL -> S3 (raw)
 -> FileUploaded event
 -> Video Processing Worker
      -> transcode renditions + HLS packaging
      -> thumbnail
 -> VideoProcessingCompleted event
 -> Notification Service (video ready)
 -> Course Service (lecture marked playable)

Student plays lecture
 -> Frontend player requests manifest (access-controlled)
 -> hls.js adapts quality to bandwidth
 -> on completion -> autoplay next lecture in course
```
...
## Workflow H — Coding Submission, Grading, and Similarity Analysis

```text
Learner submits code
 -> Assessment Service stores submission
 -> SubmissionCreated
 -> Kafka
      -> Code Execution / Grading Worker
           -> isolated execution + limits + tests
           -> GradingCompleted
      -> Content Similarity Worker
           -> tokenize/fingerprint/compare
           -> SimilarityAnalysisCompleted
 -> Assessment Service consumes results
 -> learner sees grading result
```

The compute workers are asynchronous and resource-bounded. The Go versions are required benchmark implementations; the final retained runtime is decided from measurements in Milestone 38.5.

---

# 77. Concepts We Will Be Able to Explain by the End
...
- IaC;
- disaster recovery;
- immutable payment ledgers, refund state machines, chargebacks, and access revocation;
- audit records vs application logs;
- RAG ingestion and retrieval;
- background video transcoding and multi-rendition encoding;
- adaptive bitrate streaming (HLS) and why a single MP4 can't do it;
- bandwidth-based adaptive playback on the frontend;
- assessment/submission event flows;
- resource-bounded execution of untrusted code behind an isolation boundary;
- content fingerprinting/similarity analysis;
- benchmark-driven polyglot architecture decisions rather than language-by-assumption.

---

# 78. Engineering Deliverables
...
# 79. What We Will Deliberately Avoid
...
- manage customer support tickets (see §6.7.1 for the full refund/support workflow this connects to);
- review chargebacks/disputes flagged from payment provider webhooks.
...
This is the **single authoritative progression order** for teaching and milestone status. It incorporates the complete required project and learning scope defined throughout this curriculum. Milestone summaries are additive to the detailed architecture/topic sections above; a concise milestone description must never be interpreted as permission to skip a required topic, implementation concern, test, failure mode, or production practice defined elsewhere in this document.

When `MILESTONE_STATUS.md` names a current milestone, its number/title must refer to this authoritative sequence.

## Milestone 0 — Architecture Orientation and System HLD

Monolith vs modular monolith vs microservices, why microservices are difficult, bounded contexts, service ownership and responsibility mapping, database-per-service/data ownership, sync vs async communication, first system architecture diagram, service responsibility map, and first ADRs.

## Milestone 1 — Repository and Engineering Foundation

pnpm workspace/monorepo, NestJS apps, Next.js app, shared strict TypeScript/linting/formatting configuration, test commands, environment validation, and independent buildability of every application.
...
Docker Compose, PostgreSQL first, then Kafka/Redis/local object storage incrementally as needed, with OpenSearch added when the search track begins. Teach container networking and local service discovery rather than treating Compose as a black box.

## Milestone 3 — API Standards

DTO validation, standard errors, OpenAPI conventions, request IDs, pagination, versioning conventions, a logging abstraction, and deliberately small shared technical packages. Do not share domain/ORM entities across services. The shared event-envelope technical contract is introduced and finalized in Milestone 11 rather than invented ad hoc here.

## Milestone 4 — Auth Service

Registration, login, password hashing, JWT access tokens, refresh-token rotation, logout/revocation, brute-force controls, and authorization-guard foundations.

## Milestone 5 — User Service and Authorization Foundations

Profiles, roles, ownership, RBAC/resource authorization, authenticated cross-service interaction, identity propagation, and the distinction between authentication propagation and business authorization.

## Milestone 6 — Course Service HLD and Implementation

Course aggregate, course CRUD, modules, lessons, draft/published state, instructor ownership/authorization, optimistic concurrency concepts, API, DB, and event candidates.
...
Lesson completion, course progress tracking, write-heavy workload concepts, and an explicit review of why Enrollment and Learning Progress are related but distinct service/data boundaries.
...
Topics, partitions, offsets, producers, consumers, consumer groups, ordering, replay, concrete course events, and the distinction between commands and events.
...
Concrete schema registration/validation, compatibility modes and enforcement, schema evolution, producer/consumer validation, CI compatibility checks, and compatibility tests.
...
Duplicate-safe async processing with inbox/deduplication records, safe retries, and replay-safe consumers.
...
Payment lifecycle/states, external-provider abstraction, webhook verification, idempotency, and the refund lifecycle.

## Milestone 16.1 — Production Payment Ledger, Refunds, and Customer Support (Extension)

Append-only payment ledger, refund state machine with reason codes and full audit trail, partial refunds, chargeback/dispute handling, support ticket workflow linked to learner/payment/enrollment, least-privilege support-agent role, and policy-based auto-refund eligibility window. `RefundCompleted` must be consumed by Enrollment Service to revoke/update access according to policy and by Notification Service to inform the learner. This is additive to Milestone 16 and does not change the payment work already defined there.
...
File metadata, pre-signed upload/download URLs, ownership/access control, and the file-complete/`FileUploaded` event that starts downstream processing.
...
Frontend video player using `hls.js`: adaptive bitrate switching based on bandwidth, manual quality override, resume playback position, autoplay-next-video within a course, access-controlled rendition fetching, and UI states for processing/ready/failed videos.
...
Authorization-aware retrieval, metadata filters, citations, reranking, retrieval/answer evaluation, prompt-injection defense, embedding/index versioning and reindexing, plus token/latency tracking.
...
## Milestone 35.1 — Audit / Activity Pipeline

Build the durable audit consumer defined in §6.16: capture security-sensitive activity, admin actions, and important domain transitions as append-oriented audit records; distinguish them from application logs; define actor/action/target/correlation metadata, access controls, retention, and failure/replay behavior.
...
## Milestone 38.1 — Assessment Service and Submission Workflow

Build the Node.js/NestJS Assessment Service before its compute workers: coding-exercise/test-suite metadata, submission records and states, grading policy/attempt limits, learner-facing results, `SubmissionCreated` publication, and consumption of `GradingCompleted` / `SimilarityAnalysisCompleted`. Verify that untrusted learner code is stored and described here but is **never executed inside the Assessment Service**. This milestone establishes the stable event contracts that the later benchmark implementations consume.

## Milestone 38.2 — Go Foundations and Polyglot Service Design

Teach Go from zero only as far as required for the compute services: compiled binaries, packages, structs, interfaces, errors, goroutines, channels, context cancellation, profiling basics, testing, and Kafka client usage. Compare Go's concurrency model with Node.js async I/O and worker threads.

## Milestone 38.3 — Go Code Execution / Grading Service

Build the Go benchmark implementation behind `SubmissionCreated` / `GradingCompleted`. Implement bounded concurrency, cancellation, job state, execution timeouts, resource accounting, idempotent event handling, and a sandbox abstraction. Start with a deliberately limited local backend, then define the production-isolation contract.

## Milestone 38.4 — Go Content Similarity Service

Build the Go benchmark implementation for CPU-bound similarity analysis: normalization, tokenization, fingerprints, similarity calculations, bounded parallelism, corpus partitioning, and event publication. Add benchmarks against a representative Node.js implementation.

## Milestone 38.5 — Polyglot Performance and Architecture Decision

Use the benchmark results to decide whether Go remains justified for each workload. Record throughput, p50/p95 latency, CPU utilization, memory per worker, concurrency, cost-per-job, and operational complexity. If a Go version does not materially improve its target workload enough to justify the added language/tooling, that final production service must be allowed to remain Node.js/TypeScript; the lesson is measurement, not language ideology.

## Milestone 39 — OpenTelemetry Foundation

OpenTelemetry context propagation, traces/spans, metrics, structured logs where supported, explicit service names, correlation/request IDs, and the collector pipeline.

## Milestone 40 — Prometheus and Grafana

Prometheus metric scraping and Grafana production dashboards, including API, database, Kafka, cache, worker, and WebSocket views where relevant.
...
Integration tests against real PostgreSQL/Kafka/Redis using Testcontainers or equivalent disposable test infrastructure where appropriate, including API and Kafka integration paths.

## Milestone 45 — Contract Testing

HTTP and event compatibility across independently changing services, including event-schema compatibility tests.

## Milestone 46 — End-to-End Testing

Critical complete user journeys across frontend, gateway, services, messaging, and data stores.

## Milestone 47 — Performance Testing

k6 load baselines, throughput, latency percentiles, saturation, and WebSocket load testing for the real-time path.
...
Multi-stage builds, deterministic dependency installs, efficient build contexts, non-root runtime, signal handling, graceful shutdown support, and health behavior.
...
Lint, typecheck, unit tests, integration tests, contract tests, application build, and container-image build; security/scanning controls are added in Milestone 59.
...
ECR, immutable image tags, commit-SHA artifacts, environment promotion, and artifact provenance concepts.
...
Backward-compatible migrations, migration jobs, expand/migrate/contract, release/deployment ordering, and rollback-safe compatibility.
...
Break several parts of the platform—such as killing a service, restarting a database, stopping Redis, duplicating events, slowing payment responses, killing a Kafka consumer, and terminating chat pods—and verify alerts, retries, graceful degradation, runbooks, recovery, and data correctness.

## Milestone 76 — Final Production Readiness Review

Perform the final architecture, reliability, security, testing, delivery, data, operations, and cost review. Produce the final architecture diagram, service catalog, API catalog, event catalog, ADR set, threat model, SLOs, dashboards, runbooks, backup/restore plan, deployment guide, scaling guide, and production-readiness checklist so the repository is a portfolio-quality architecture case study rather than only source code.
...
- claiming Go is required merely because work is CPU-intensive without benchmarking Node worker threads;
- executing untrusted learner code in the Go service process or API pod;
- scaling CPU workers from HTTP traffic instead of queued work/saturation;
- allowing CPU-heavy workers to starve latency-sensitive services;
...
# Initial Next Step (Only When the Project Is NOT STARTED)

This is a bootstrap default, **not live project status**. If `MILESTONE_STATUS.md` already records progress, ignore this section and resume from the exact next teaching/engineering steps in the persistent state files and current repository.
...
We stop at the end of Milestone 0, grade the mandatory assignment, apply the Milestone Completion Gate, update the persistent state files, and only then continue when the learner chooses to proceed.

# 80. Core Polyglot Learning Track — Two CPU/Concurrency-Intensive Benchmark Implementations

This track is part of the main project, not an optional extension. The primary platform remains Node.js/TypeScript for business APIs and orchestration. Two bounded contexts are implemented in Go as **required benchmark/learning implementations** because their workloads are plausible candidates for a separate runtime; the final production architecture retains Go only if Milestone 38.5 measurements justify the additional language and tooling. This keeps the learning objective (real polyglot implementation and comparison) mandatory without prejudging the architecture decision.

## Why Go is not chosen by assumption

Node.js does have mechanisms for CPU-intensive JavaScript, including `worker_threads`. Therefore the mentor must explicitly benchmark before declaring Go necessary. The decision is based on workload characteristics and operational requirements, not the simplistic rule that "Node is bad at CPU."

The benchmark must compare at least:

- throughput at a fixed CPU budget;
- p50/p95 execution latency;
- memory per concurrent job;
- concurrency level before saturation;
- startup time for short-lived jobs;
- operational complexity;
- cost per 1,000 representative jobs.

The benchmark becomes an ADR-backed architecture decision.

## Go Use Case 1 — Code Execution / Auto-Grading

A learner submits source code. The Assessment Service stores the submission and emits `SubmissionCreated`. The Go service consumes the event, schedules an isolated execution, enforces CPU/memory/time/output limits, runs hidden tests, records the result, and publishes `GradingCompleted`.

Mental model:

```text
Node.js Assessment Service
        |
        | SubmissionCreated
        v
Kafka
...
The service must be designed so the Kafka contract stays stable even if the sandbox implementation changes from a local development backend to a stronger production isolation mechanism.

## Go Use Case 2 — Content Similarity / Plagiarism Analysis

A submission or uploaded learning artifact is compared against a large corpus. The Go service performs tokenization, fingerprinting, similarity calculations, and bounded parallel comparisons inside its own process, then publishes an analysis result.
...
Kafka
        |
        v
Go Similarity Service
        |
        +--> normalize
        +--> tokenize
        +--> fingerprint
        +--> parallel similarity work
        +--> persist result
        |
        v
SimilarityAnalysisCompleted
...
This is intentionally different from the video worker. Video transcoding is delegated to FFmpeg, so the CPU-heavy work is performed by a native process. It is therefore useful for learning process orchestration, but it does not by itself prove that a second application language is required.

## Ground-up Go teaching scope

The first time Go appears, teach:

- what a compiled, statically typed language is;
- packages and `func main()`;
- variables, structs, methods, and interfaces;
- explicit error returns;
- `context.Context` for cancellation/deadlines;
- goroutines;
- channels;
- worker pools and bounded concurrency;
- Kafka consumer/producer usage;
- profiling concepts such as CPU and heap profiles;
- `go test` and benchmark tests;
- building a static binary and a minimal container image.

Do not turn this into a generic Go course. Every language lesson must connect directly to one of the two production workloads.

## Production requirements for both Go services

Each Go service must eventually have:

- its own deployable artifact;
- its own configuration and secrets;
- its own health/readiness endpoints;
- structured logs;
...
- failure-path tests;
- Docker image scanning and SBOM generation;
- independent CI checks;
- a runbook for saturation and backlog recovery.

The code execution service additionally requires a documented threat model and isolation policy before it is allowed to process untrusted code.
...
- a dedicated external/standalone feature-flag platform beyond the core Milestone 64 implementation;
- video DRM (Widevine/FairPlay/PlayReady) and forensic watermarking for copy-proof playback (see §30.4 for why this is out of core scope).
...
| --------------------------- | ----------------------------------------------------------------------- |
| Backend                     | NestJS + TypeScript; Go benchmark implementations for designated compute-heavy services, retained where measurements justify the split |
| Frontend                    | Next.js + TypeScript                                                    |
...
# 87. Our First Actual Build Session (Initial Bootstrap Only)

Use this section only when the persistent tracking files say the project is **NOT STARTED**. Otherwise, the session-resume protocol overrides it and the mentor continues from the recorded current milestone.

For a brand-new project, after this roadmap is accepted, we start with:
...
**We will not start coding until those boundaries make sense and the Milestone 0 assignment has demonstrated the required understanding.**

---

# 88. Session Ending Convention

This section restates the **Teaching Contract** and does not replace it with a shorter template.

Every milestone response ends with:
...
One or two short reasoning questions when useful.

## Assignment

A scoped production-relevant task that requires the learner to produce, decide, debug, trace, or justify something. It is graded using the mandatory Assignment Review format.
...
Then stop. Do not start the next milestone until the assignment has been graded, the Milestone Completion Gate has been applied, and the learner chooses to proceed. If this is also the end of a substantial learning session, update `MILESTONE_STATUS.md`, `LEARNING_STATE.md`, and `DECISIONS.md` when required before stopping.
...
# 90. Starting Point (Initial Bootstrap Only)

This section applies only when the project has no recorded progress yet. In every resumed session, `MILESTONE_STATUS.md` and `LEARNING_STATE.md` determine the actual continuation point.

**Initial milestone: Milestone 0 — Architecture Orientation and System HLD.**

For a brand-new project, begin only after the learner confirms this roadmap is the intended scope. Once work has started, do not return to this static starting point unless the learner explicitly requests a reset/review.
