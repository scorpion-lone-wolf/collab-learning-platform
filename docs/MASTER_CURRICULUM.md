...
> 6. This applies retroactively to every milestone in this document — original, superseded/legacy, and newly added extension milestones alike. No milestone is exempt because it looks "simple" (e.g. Milestone 0/1) or "advanced" (e.g. Kubernetes/Terraform milestones) — advanced-sounding milestones get exactly the same ground-up treatment, not less.
>
...
flowchart TB
    ...
    Edge --> Similarity[Content Similarity Service]
    ...
    Similarity --> SimilarityDB[(Similarity DB)]
    ...
    GoRunner[Go Code Execution / Grading Service]
    GoSimilarity[Go Content Similarity Service]
    Kafka <--> GoRunner
    Kafka <--> GoSimilarity
    GoRunner --> RunnerDB[(Execution DB)]
    GoSimilarity --> SimilarityDB
...
This extends §6.7 and is implemented in Milestone 13.1 (legacy list) / Milestone 16.1 (authoritative list). Nothing above is changed. This closes the gap between "a Payment Service exists" and "money is handled the way a real production system handles it."
...
This is a core polyglot service, not an optional technology demo. It exists because the platform now has a real workload involving many concurrent, resource-bounded executions.
...
This is the second core Go service and provides a genuine CPU-intensive workload executed inside the service process rather than delegated entirely to another native tool.
...
This extends Section 30 and Milestone 24 (File Service) and is implemented in Milestones 24.1–24.2. Nothing above is changed.
...
This extends §30.3 and directly answers: *"only a user who paid should be able to watch."* It ties the video pipeline back to the Payment (Milestone 13), Enrollment (Milestone 7), and Saga (Milestone 14) work already defined elsewhere in this document. Nothing above is changed.
...
## Milestone 13.1 — Production Payment Ledger, Refunds, and Customer Support (Extension)

Implement:

- append-only payment ledger (authorization/capture/refund/chargeback/adjustment rows, not a single mutable status field);
- refund state machine (requested → under_review → approved → processing → completed, plus rejected) with reason codes and full audit trail;
- partial refund support;
- chargeback/dispute webhook handling, distinct from normal refunds;
- support ticket data model linked to learner/payment/enrollment;
- support-agent role with scoped, least-privilege access to payment history and refund actions;
- automatic policy-based refund approval within an eligibility window, bypassing manual review;
- `RefundCompleted` event consumed by Enrollment Service (revoke access) and Notification Service (inform learner).

Note: mirrors Milestone 16.1 in the Authoritative Step-by-Step Milestone Sequence, which is the sequence the mentor actually follows. This entry keeps the legacy roadmap in sync as reference material.
...
## Milestone 16.1 — Video Processing Pipeline (Extension)

Implement:

- background transcoding worker consuming file-processing events;
- multiple quality renditions (360p–1080p);
- HLS packaging for adaptive streaming;
- thumbnail generation;
- processing status tracking with retry;
- video-processing-complete/failed events wired to notifications.

Note: this section mirrors Milestones 24.1–24.2 in the Authoritative Step-by-Step Milestone Sequence, which is the sequence the mentor actually follows. This entry exists here only to keep this legacy roadmap in sync as reference material.

---

## Milestone 16.2 — Adaptive Video Player (Extension)

Implement:

- HLS.js integration in the Next.js frontend;
- bandwidth-based automatic quality switching with manual override;
- resume-from-last-position;
- autoplay-next-video within a course;
- processing/ready/failed UI states.
...
Append-only payment ledger, refund state machine with reason codes and audit trail, partial refunds, chargeback/dispute handling, support ticket workflow, least-privilege support-agent role, policy-based auto-refund eligibility window. This is additive to Milestone 16 and does not change the payment work already defined there.
...
## Milestone 48.1 — Go Foundations and Polyglot Service Design
...
## Milestone 48.2 — Go Code Execution / Grading Service

Build the Go service behind `SubmissionCreated` / `GradingCompleted`. Implement bounded concurrency, cancellation, job state, execution timeouts, resource accounting, idempotent event handling, and a sandbox abstraction. Start with a deliberately limited local backend, then define the production-isolation contract.

## Milestone 48.3 — Go Content Similarity Service

Build CPU-bound similarity analysis inside the Go process: normalization, tokenization, fingerprints, similarity calculations, bounded parallelism, corpus partitioning, and event publication. Add benchmarks against a representative Node.js implementation.

## Milestone 48.4 — Polyglot Performance and Architecture Decision

Use the benchmark results to decide whether Go remains justified. Record throughput, p50/p95 latency, CPU utilization, memory per worker, concurrency, cost-per-job, and operational complexity. If the Go version does not materially improve the target workload, the service must be allowed to remain Node.js; the lesson is measurement, not language ideology.
...
# 81.1 Core Polyglot Track — Two CPU/Concurrency-Intensive Services in Go

This track is part of the main project, not an optional extension. The primary platform remains Node.js/TypeScript for business APIs and orchestration, while two bounded contexts are implemented in Go because their measured workload characteristics justify a separate runtime.
...
| --------------------------- | ----------------------------------------------------------------------- |
| Backend                     | NestJS + TypeScript; Go for designated compute-heavy services          |
| Frontend                    | Next.js + TypeScript                                                    |
